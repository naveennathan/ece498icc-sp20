...
// Compute C = A * B
__global__ void matrixMultiplyNormal(float *A, float *B, float *C, 
  ...
  for (int i = 0; i < numARows; i++) {
    for (int j = 0; j < numBColumns; j++) {
      ...
      /* Write Pvalue to corresponding cell in matrix C */
...
int main(int argc, char **argv) {
  ...
  matrixMultiplyNormal<<<dimGrid, dimBlock>>>(deviceA, deviceB, deviceC, 
                                              numARows, numAColumns, 
                                              numBRows, numBColumns,
                                              numCRows, numCColumns);
